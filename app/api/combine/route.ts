...
export async function GET(request: NextRequest) {
  ...
  const chatCompletion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `TASK: Combine ${element1} and ${element2} to create a new element. Try to keep the element as simple and realistic as possible and only 1 word if possible as well. If two basic elements are combined, you should prioritize making a new thing out of that, rather than simply combining the words. Example: Earth + Earth = Solar System. You are allowed to use one of the inputs as the output, but only if there are no realistic elements. Two of the same item should output a larger version of that item if applicable. Your response should be the name of the new element and MUST contain one and only one emoji to represent the element. The response should never have less than or more than 1 emoji. Example: Eden + Orchard = 🍎 Apple. Prioritize names of new elements that are as common as possible. Prioritize organic elements or types of food as new elements. Make sure the new element  is not just a combination of the two words, but something unique and different. Your output should be in json format to be parsed. Format: {new_element: "name", emoji: "emoji"}`,
      },
    ],
  });
